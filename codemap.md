...
- `api/src/index.ts`: Backend application bootstrap, middleware registration, route definition, and server startup.
...
2. The `api/` package accepts HTTP traffic, logs requests, and routes authentication traffic into better-auth backed by PostgreSQL.
